...
class RegularizedRegression(object):
    ...
    def poly_feature_transform(self,X,poly_order=1):
        ...
        if poly_order==1:
          ...
          f_transform = np.concatenate(([f_transform**(i+1) for i in range(poly_order)]), axis=1)
        ...
        append_ones = np.ones((f_transform.shape[0], 1))
        f_transform = np.concatenate((f_transform, append_ones), axis=1)
    ...
    def train_analytic(self, X, y, poly_order=1, lambda_reg=0):
        ...
        L = np.identity(poly_order+1)
        X = self.poly_feature_transform(X, poly_order)
        self.params['W'] = np.linalg.pinv(X.T@X + lambda_reg*L)@X.T@y
    ...
    def loss(self, X, y=None, lambda_reg=0.0):
        ...
        loss = (1/(2*N)) * np.sum(np.square(prediction-y)) + (1/2) * lambda_reg * sum(np.square(W))
        ...
        grads['W'] = (1/N) * X.T@(prediction-y) + lambda_reg * W
