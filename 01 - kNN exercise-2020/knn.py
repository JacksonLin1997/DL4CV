...
class KNearestNeighbor(object):
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            sort_d = np.argsort(dists[i])
            for top_k in range(k):
              closest_y.append(self.y_train[sort_d[top_k]])
            
            y_pred[i] = np.argmax(np.bincount(closest_y))
            # by default, it already choose the smaller label if ties
    ...
    def compute_distances_no_loops(self, X):
        ...
        # (x-y)^2 = x^2 - 2xy + y^2
        test_square = np.sum(X**2, axis=1)[:, np.newaxis]
        train_square = np.sum(self.X_train**2, axis=1)
        test_multiply_train = 2 * np.dot(X, self.X_train.T)
        dists = np.sqrt( test_square - test_multiply_train + train_square )
