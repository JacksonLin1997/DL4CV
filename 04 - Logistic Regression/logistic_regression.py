...
class LogisticRegression(object):
    ...
    def predict(self, X):
        ...
        prediction = np.where(self.sigmoid( X@W + b ) >0.5, 1, 0)
    ...
    def binary_cross_entropy(self, probs, labels):
        ...
        m = probs.shape[0]
        bce = np.squeeze( (1/m) * (-labels.T@np.log(probs) - (1-labels).T@np.log(1-probs)) )
    ...
    def loss(self, X, y=None):
        ...
        predict_prob = self.sigmoid( X@W + b )
        ...
        loss = self.binary_cross_entropy(predict_prob, y)
        ...
        grads['W'] = (1/N) * X.T@(predict_prob-y)
        grads['b'] = (1/N) * np.sum(predict_prob-y)
