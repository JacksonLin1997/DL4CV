...
class MultinomialLogisticRegression(object):
    ...
    def softmax(self,x):
        ...
        const = x-np.max(x, axis=1, keepdims=True)
        probs = np.exp(const) / np.sum(np.exp(const), axis=1, keepdims=True)
    ...
    def predict(self, X):
        ...
        class_prob = self.softmax( X@self.params['W'] + self.params['b'] )
    ...
    def loss(self, X, y=None, lambda_reg=0.0):
        ...
        log_like = -np.log(score[range(N), y])
        loss = np.sum(log_like) / N + lambda_reg * np.sum(W**2)
