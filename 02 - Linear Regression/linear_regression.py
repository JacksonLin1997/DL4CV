...
class LinearRegression(object):
    ...
    def feature_transform(self, X):
        ...
        new_dim = np.ones((num_samples, 1))
        X_transformed = np.c_[X, new_dim]
    ...
    def train_analytic(self, X, y):
        ...
        new_X = self.feature_transform(X)

        # theta = inv(X^T * X) * X^T * y
        XT = new_X.T
        XTX = XT@new_X
        XTX_inverse = np.linalg.pinv(XTX)
        self.params['W'] = (XTX_inverse@XT)@y
    ...
    def loss(self, X, y=None):
        ...
        loss = (1/(2*N)) * np.sum(np.square(prediction-y))
    ...
    def train(self, X, y, X_val=None, y_val=None,learning_rate=1e-3, learning_rate_decay=0.95,
        ...
        for it in range(num_iters):
            ...
            minibatch = np.random.choice(X.shape[0], batch_size, replace=False)
            X_batch = X[minibatch, :]
            y_batch = y[minibatch, :]
